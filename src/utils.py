...
def load_beir_datasets(dataset_name, split):
    supported_remote_datasets = ['nq', 'msmarco', 'hotpotqa']
    if dataset_name == 'msmarco': split = 'train'
    url = "https://public.ukp.informatik.tu-darmstadt.de/thakur/BEIR/datasets/{}.zip".format(dataset_name)
    out_dir = os.path.join(os.getcwd(), "datasets")
    data_path = os.path.join(out_dir, dataset_name)
    if not os.path.exists(data_path):
        if dataset_name not in supported_remote_datasets:
            raise ValueError(
                f"Unknown dataset {dataset_name!r}: expected a local BEIR directory at {data_path}."
            )
