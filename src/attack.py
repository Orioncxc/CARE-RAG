...
class Attacker():
    def __init__(self, args, **kwargs) -> None:
        ...
        self.args = args
        self.device = getattr(args, "runtime_device", "cuda")
    ...
    def hotflip(self, target_queries, adv_b=None, **kwargs) -> list:
        ...
        for query_score in tqdm(target_queries):
            ...
            for j in range(self.adv_per_query):
                ...
                adv_passage_ids = torch.tensor(adv_passage, device=self.device).unsqueeze(0)
                adv_passage_attention = torch.ones_like(adv_passage_ids, device=self.device)
                adv_passage_token_type = torch.zeros_like(adv_passage_ids, device=self.device)  
                ...
                q_sent = {key: value.to(self.device) for key, value in q_sent.items()}
                ...
                for it_ in range(self.num_iter):
                    ...
                    candidates = hotflip_attack(grad[token_to_flip],
                                                ...
                                                filter=None)                
                    current_score = 0
                    candidate_scores = torch.zeros(self.num_cand, device=self.device) 
