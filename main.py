...
def main():
    args = parse_args()
    device = resolve_runtime_device(args.gpu_id)
    args.runtime_device = str(device)
    ...
    for iter in range(args.repeat_times):
        ...
        if args.attack_method not in [None, 'None']:
            ...
            adv_input = move_inputs_to_device(adv_input, device)
            with torch.no_grad():
        ...
        for i in target_queries_idx:
            ...
            if args.use_truth == 'True':
                ...
                if args.attack_method not in [None, 'None']: 
                    ...
                    query_input = move_inputs_to_device(query_input, device)
                    with torch.no_grad():
